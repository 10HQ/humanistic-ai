...
class ArgumentAnalyzer:
    ...
    def __init__(self):
        ...
        self.authority_patterns = [
            (r'(专家|教授|博士|权威|研究|科学家)\s*(认为|指出|表示|证明)', '引用权威但未提供具体证据'),
        ]
        ...
        self.ad_hominem_patterns = [
            (r'(你|他|她|他们|对方)\s*(就是|不过|只是)\s*(一个|个)\s*(.*?)(人|家伙|货)', '攻击人格特质'),
            (r'(你|他|她)\s*(懂什么|有什么资格|凭什么)\s*(说|评论|批评)', '质疑对方资格'),
            (r'一个连.*?都没有的人', '以身份否定论点'),
        ]
        
        self.straw_man_patterns = [
            (r'(你|你们|对方)\s*(的意思|的观点)\s*(就是|无非是|不过是)\s*(.+?)，\s*(但|然而)', '歪曲对方观点'),
        ]
        
        self.emotion_patterns = [
            (r'(难道|怎能|岂能)\s*(不|没有)\s*(.*?)(吗|呢|？)', '修辞性反问替代论证'),
            (r'(想想|试想|想象一下)\s*(.*?)(的|之)\s*(后果|结果)', '诉诸恐惧'),
        ]
