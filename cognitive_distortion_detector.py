    ...
    def _load_patterns(self) -> None:
        ...
        self.patterns[DistortionType.CATASTROPHIZING] = DistortionPattern(
            ...
            patterns=[
                ...
                r"这(是|就是).*?(灾难|毁灭|末日)",
                r"(彻底|完全).*?(完蛋|失败|没希望)",
                r"(人生|这辈子|一生).*?(完了|没希望|毁了)",
                r"(再也|永远).*?(不会|不能|找不到)",
                r"(孤独终老|不会再爱|永远都不)",
                r"如果.*?(分手|失去|离开).*?(就|会).*?(完了|没希望|毁)"
            ],
        ...
        self.patterns[DistortionType.OVERGENERALIZATION] = DistortionPattern(
            ...
            keywords=["总是", "每次都", "从来都", "所有人",
                      "所有事", "每次", "永远", "全都",
                      "没有做任何事", "没有任何才能", "什么都做不好"],
            patterns=[
                r"(每次|总是|从来).*?(都|就).*?(这样|如此)",
                r"(所有|全部).*?(人|事|情况).*?(都|就)",
                r"一(次|个).*?(就|都).*?(说明|代表|证明)",
                r"从(来|没有).*?(成功|做好|对过)",
                r"(没有|毫无).*?(任何|做任何).*?(才能|能力|本事)",
                r"(说明|证明).*?(我|自己).*?(没有|不行|不能)",
                r"(永远|一直).*?(都|会).*?(这样|如此|不行)",
                r"(什么都|任何事).*?(做不好|不行|失败)"
            ],
            questioning_templates=[
                "这个结论是基于多少证据？有没有反例？",
        ...
        self.patterns[DistortionType.MIND_READING] = DistortionPattern(
            ...
            keywords=["他觉得", "他认为", "他们觉得", "别人认为",
                      "肯定是想", "一定是觉得", "看不起", "讨厌我",
                      "一定是生我的气", "不想理我", "对我有意见", "想开除我"],
            patterns=[
                r"(他|她|他们|别人|领导|老师|同事).*?(觉得|认为|想).*?(肯定|一定|绝对)",
                r"(肯定|一定|绝对).*?(看不起|讨厌|不喜欢|嫌弃)",
                r"明摆着.*?(是|就是).*?(想|要|觉得)",
                r"(一定|肯定).*?(是|就是).*?(生.*?气|不.*?理|有意见)",
                r"(不想|不愿).*?(理|见|搭理).*?(我|我们)",
                r"(肯定|一定).*?(对我|觉得我).*?(有意见|不满|不好的看法)",
                r"(想|打算|准备).*?(开除|辞退|赶走).*?(我|我们)",
                r"(没有|没).*?(回|回复|回应).*?(消息|电话|微信).*?(一定|肯定).*?(生气|不高兴)"
            ],
            questioning_templates=[
                "你有什么证据支持这个判断？",
                "有没有其他可能的解释？",
