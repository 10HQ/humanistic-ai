...
class EthicsChecker:
    ...
    RED_LINE_PATTERNS = {
        "mass_surveillance": [r"大规模.*监控|全面.*监视|全民.*监控"],
        "autonomous_weapons": [r"自主.*武器|自动.*杀伤|无人.*武器.*决策"],
        "social_credit": [r"社会信用.*评分|行为.*评分.*不可申诉"],
        "behavior_manipulation": [r"不知情.*行为.*改变|暗中.*操纵.*行为"]
    }
